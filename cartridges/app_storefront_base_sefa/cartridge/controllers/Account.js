...
server.get(
    ...
    function (req, res, next) {
        ...
        accountModel.profile.loyaltyId = customer.profile.custom.loyaltyid;
...
/**
 * Account-EditProfile : The Account-EditProfile endpoint renders the page that allows a shopper to edit their profile. The edit profile form is prefilled with the shopper's first name, last name, phone number and email
 * @name Base/Account-EditLoyaltyId
 * @function
 * @memberof Account
 * @param {middleware} - server.middleware.https
 * @param {middleware} - csrfProtection.generateToken
...
function (req, res, next) {
    ...
    profileForm.clear();
    profileForm.LoyaltyIdForm.loyaltyId.value = oldLoyaltyId;
    res.render('account/loyaltyId', {
        consentApi: Object.prototype.hasOwnProperty.call(req.session.raw, 'setTrackingAllowed'),
        caOnline: content ? content.online : false,
        profileForm: profileForm,
        breadcrumbs: [
...
server.post(
    ...
    function (req, res, next) {
        ...
        var oldLoyaltyId = profileForm.LoyaltyIdForm.loyaltyId.htmlValue;
        var customer = CustomerMgr.getCustomerByCustomerNumber(req.currentCustomer.profile.customerNo);
