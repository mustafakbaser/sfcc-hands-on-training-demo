...
function genericExport(args, callback) {
    ...
    var targetFolder = File.IMPEX + FileHelper.prependSeparator(StepUtil.replacePathPlaceholders(args.TargetFolder));
    ...
    var filePath = [targetFolder, filename].join(File.SEPARATOR);

    var status = new Status(Status.OK, 'OK', 'Export successful');

    // Call the callback function, this will trigger the actual export
    var result = callback(filePath.replace([File.IMPEX, 'src'].join(File.SEPARATOR), ''), args);
...
var content = function content(options) {
    var callback = function callback(file, args) {
        ...
        if (!empty(foldersToExport)) {
            ...
            foldersIterator = new ArrayList([ContentMgr.getFolder(library.getID(), 'root')]).iterator(); // Export all folders from the 'root'
        }
...
var coupons = function coupons(options) {
    var callback = function callback(file, args) {
        ...
        if (!empty(couponsToExport)) {
            ...
            couponsIterator = CouponMgr.getCoupons().iterator(); // Export all coupons if none is provided
        }
...
var customerList = function customerList(options) {
    var callback = function callback(file, args) {
        ...
        try {
            profilesIterator = CustomerMgr.searchProfiles(StepUtil.replaceQueryPlaceholders(query), 'customerNo DESC');
            ...
            return new Status(Status.ERROR, 'ERROR', 'Failed to export the customer list: ' + e);
        } finally {
            profilesIterator && profilesIterator.close();
        }
    };
...
var customers = function customers(options) {
    var callback = function callback(file, args) {
        ...
        try {
            ...
            return new Status(Status.ERROR, 'ERROR', 'Failed to export the customers: ' + e);
        } finally {
            profilesIterator && profilesIterator.close();
        }
    };
...
var customObjects = function customObjects(options) {
    var callback = function callback(file, args) {
        ...
        try {
            customObjectsIterator = CustomObjectMgr.queryCustomObjects(type, StepUtil.replaceQueryPlaceholders(query), 'UUID DESC');
            ...
            return new Status(Status.ERROR, 'ERROR', 'Failed to export the custom objects: ' + e);
        } finally {
            customObjectsIterator && customObjectsIterator.close();
        }
    };
...
var giftCertificates = function giftCertificates(options) {
    var callback = function callback(file, args) {
        ...
        try {
            giftCertificatesIterator = SystemObjectMgr.querySystemObjects('GiftCertificate', StepUtil.replaceQueryPlaceholders(query), 'UUID DESC');
            ...
            return new Status(Status.ERROR, 'ERROR', 'Failed to export the gift certificates: ' + e);
        } finally {
            giftCertificatesIterator && giftCertificatesIterator.close();
        }
    };
...
var orders = function orders(options) {
    var callback = function callback(file, args) {
        ...
        try {
            ordersIterator = OrderMgr.searchOrders(StepUtil.replaceQueryPlaceholders(query), 'orderNo DESC');
...
var productLists = function productLists(options) {
    var callback = function callback(file, args) {
        ...
        try {
            productListsIterator = ProductListMgr.queryProductLists(StepUtil.replaceQueryPlaceholders(query), 'UUID DESC');
            ...
            return new Status(Status.ERROR, 'ERROR', 'Failed to export the product lists: ' + e);
        } finally {
            productListsIterator && productListsIterator.close();
        }
    };
...
var sourceCodes = function sourceCodes(options) {
    var callback = function callback(file, args) {
        ...
        try {
            sourceCodeGroupsIterator = SystemObjectMgr.querySystemObjects('SourceCodeGroup', StepUtil.replaceQueryPlaceholders(query), 'UUID DESC');
            ...
            return new Status(Status.ERROR, 'ERROR', 'Failed to export the source codes: ' + e);
        } finally {
            sourceCodeGroupsIterator && sourceCodeGroupsIterator.close();
        }
    };
...
var stores = function stores(options) {
    var callback = function callback(file, args) {
        ...
        try {
            storesIterator = SystemObjectMgr.querySystemObjects('Store', StepUtil.replaceQueryPlaceholders(query), 'UUID DESC');
            ...
            return new Status(Status.ERROR, 'ERROR', 'Failed to export the stores: ' + e);
        } finally {
            storesIterator && storesIterator.close();
        }
    };
