...
server.post('AddProduct', function (req, res, next) {
    ...
    //ProductMgr library defined
    var ProductMgr = require('dw/catalog/ProductMgr');
    //Get product with productId
    var product = ProductMgr.getProduct(productId);
    //Product size defined to productSize variable
    var productSize = product.custom.size;
    //Cookie library defined
    var Cookie = require('dw/web/Cookie');
    //Cookie defined
    var customCookie = Cookie("productSize", productSize);
    customCookie.setMaxAge(500);
    customCookie.setHttpOnly(true);
    response.addHttpCookie(customCookie);
    if (currentBasket) {
