...
/**
 * Account-EditLoyaltyId : The Account-EditLoyaltyId endpoint renders edit loyaltyid page.
 */
...
server.get(
    ...
    function (req, res, next) {
        ...
        var oldLoyaltyId = customer.profile.custom.loyaltyid;
        var profileForm = server.forms.getForm('profile');
        profileForm.clear();
        res.render('account/editLoyaltyId', {
            profileForm: profileForm,
            oldLoyaltyId: oldLoyaltyId,
            breadcrumbs: [
                {
                    htmlValue: Resource.msg('global.home', 'common', null),
                    url: URLUtils.home().toString()
...
server.post(
    ...
    function (req, res, next) {
        ...
        var URLUtils = require('dw/web/URLUtils');
        var formErrors = require('*/cartridge/scripts/formErrors');
        var loyaltyForm = server.forms.getForm('profile');


        var oldLoyaltyId = loyaltyForm.loyalty.oldLoyaltyId.htmlValue;
        var customer = CustomerMgr.getCustomerByCustomerNumber(req.currentCustomer.profile.customerNo);
        var formInfo = res.getViewData()
        //form validation
       
        const regex = new RegExp('^[A-Za-z0-9]{8,255}$');
        var regexTest = regex.test(oldLoyaltyId)
        if (!regexTest) {
            res.json({
                success: false,
                abc: formInfo.profileForm,
                fields: formErrors.getFormErrors(loyaltyForm)
            })
            return next()
        }
        Transaction.wrap(function () {
            customer.profile.custom.loyaltyid = oldLoyaltyId;
        });

        res.redirect(URLUtils.url('Account-Show').toString()) //Redirect
        return next();
    }
);
