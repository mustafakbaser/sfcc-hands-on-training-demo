...
server.post('Subscribe', server.middleware.https, function (req, res, next) {
    ...
    var myForm = req.form;
    var reCaptchaCheck = myForm.hasOwnProperty('g-recaptcha-response');

    var isValidEmailid = emailHelper.validateEmail(myForm.contactEmail);
    if (isValidEmailid && reCaptchaCheck) {
        ...
        if (reCaptchaCheck == false) {
            res.json({
                error: true,
                msg: Resource.msg('subscribe.to.recaptcha.invalid', 'contactUs', null)
            });
        } else {
            res.json({
                error: true,
